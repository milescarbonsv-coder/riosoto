...
export function PageHeader({ tag, title, description, centered = false }: PageHeaderProps) {
  return (
    <div className="relative py-24 md:py-32 px-6 bg-gradient-to-br from-amber-50/40 via-white to-rose-50/20 overflow-hidden">
      {/* Decorative */}
      <div className="absolute top-0 right-0 w-96 h-96 bg-rojo/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/3" />
      ...
      <div className={`max-w-7xl mx-auto relative z-10 ${centered ? 'text-center' : ''}`}>
        ...
        <p className={`text-xl text-slate-500 max-w-2xl leading-relaxed ${centered ? 'mx-auto' : ''}`}>{description}</p>
      </div>
    </div>
  );
}
